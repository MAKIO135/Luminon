...
// Spatialisation son
final int[] group1 = { 
  0, 1, NB_PNX_WALL, NB_PNX_WALL+1
};
final int[] group2 = { 
  1, 2, 3, 6, 7, 8
};
final int[] group3 = { 
  NB_PNX_WALL-1, NB_PNX_WALL-2, NB_PNX_WALL*2-2, NB_PNX_WALL*2-1
};
int moy1, moy2, moy3;
float easingMoySon = 0.01;
float beatTimer;
float beatTimerMin = 4000;// temps min entre 2 beat en millisecondes
// Beat Detection
Minim minim;
AudioInput in;
BeatDetect beat;
...
void draw() {
  beat.detect(in.mix);
  if ( beat.isOnset() && millis()-beatTimer > beatTimerMin) {
    frameCount = 0;
    beatTimer = millis();
  }
...
void sendOsc() {
  for (int i=0, len=pnx.length; i<len; i++) {
    ...
    if (i>4) myMessage = new OscMessage("/Pano"+(i+2));
    else myMessage = new OscMessage("/Pano"+(i+1));
